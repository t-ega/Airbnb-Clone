class Reservation < ApplicationRecord
  ...
  validates :checkout_date,
            presence: true,
            comparison: {
              greater_than: :checkin_date
            }
  validate :overlapping_reservations, on: %i[create]
  ...
  def overlapping_reservations
    overlapping =
      Reservation.where(
        "property_id = ? AND (checkin_date <= ? AND checkout_date >= ?)",
        property_id,
        checkout_date,
        checkin_date
      ).exists?
    ...
    if overlapping
      self.errors.add(
        :checkin_date,
        "A reservation already exists for either of the checkin or checkout date"
      )
      self.errors.add(
        :checkout_date,
        "A reservation already exists for either of the checkin or checkout date"
